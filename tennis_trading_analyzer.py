...
def api_get(params):
    ...
    try:
        ...
        if d.get("success") == 1:
            return d.get("result", [])
        st.warning(f"API: {d.get('error', 'Sem resultado')}")
        return []
...
def search_player(name):
    return api_get({"method": "get_players", "player_name": name})
...
def get_events(player_key, days_back=200):
    ...
    return api_get({"method": "get_events", "from": date_from, "to": date_to, "player_key": str(player_key)})

@st.cache_data(ttl=1800, show_spinner=False)
def get_h2h(k1, k2):
    return api_get({"method": "get_H2H", "first_player_key": str(k1), "second_player_key": str(k2)})

# ── PROCESSING ────────────────────────────────
def parse_matches(events, player_name, surface):
    matches = []
    name_part = player_name.split(".")[-1].strip().lower()
    for ev in events:
        if not isinstance(ev, dict): continue
        status = str(ev.get("event_status","")).lower()
        if status not in ["finished","after penalties","aet","retired"]: continue
        p1 = str(ev.get("event_first_player",""))
        p2 = str(ev.get("event_second_player",""))
        is_p1 = name_part in p1.lower()
        is_p2 = name_part in p2.lower()
        ...
        ev_surf = str(ev.get("surface") or ev.get("tournament_surface") or "Hard").strip().capitalize()
        if ev_surf in ["", "None", "null"]: ev_surf = "Hard"
        matches.append({
            "date": str(ev.get("event_date",""))[:10],
            "opponent": opponent,
            "tourney": str(ev.get("tournament_name", ev.get("league_name","?"))),
            "round": str(ev.get("tournament_round", ev.get("league_round",""))),
            "surface": ev_surf,
            "result": result,
            "score": score_str,
            "scores_raw": scores,
...
def season_stats(player_detail, surface):
    stats_list = player_detail[0].get("stats", []) if player_detail else []
    ...
    out = {"rank":None,"surf_won":0,"surf_lost":0,"total_won":0,"total_lost":0,"titles":0}
    for s in stats_list:
...
def edge_score(p1sf, p2sf, p1me, p2me, p1st, p2st, n1, n2):
    s1, s2 = 0, 0
    d = abs(p1sf - p2sf)
    if p1sf > p2sf: s1 += min(35, d * 0.45)
    else: s2 += min(35, d * 0.45)
    def wr(st): w,l=st["total_won"],st["total_lost"]; return w/(w+l)*100 if (w+l)>0 else 50
    d2 = abs(wr(p1st) - wr(p2st))
    if wr(p1st) > wr(p2st): s1 += min(20, d2*0.4)
    else: s2 += min(20, d2*0.4)
    if p1me["unstable"] and not p2me["unstable"]: s2 += 25
    elif p2me["unstable"] and not p1me["unstable"]: s1 += 25
    else:
        d3 = abs(p1me["tb_pct"] - p2me["tb_pct"])
        if p1me["tb_pct"] > p2me["tb_pct"]: s1 += min(15, d3*0.4)
        else: s2 += min(15, d3*0.4)
    d4 = abs(p1me["d_pct"] - p2me["d_pct"])
    if p1me["d_pct"] > p2me["d_pct"]: s1 += min(20, d4*0.3)
    else: s2 += min(20, d4*0.3)
    tot = s1+s2
    if tot == 0: p1p,p2p = 50,50
    else: p1p=round(s1/tot*100); p2p=100-p1p
    gap = abs(p1p-p2p)
    fav = n1.split(".")[-1].strip() if p1p>p2p else n2.split(".")[-1].strip()
...
def hand_txt(h1, h2, n1, n2, surf):
    l1=n1.split(".")[-1].strip(); l2=n2.split(".")[-1].strip()
    if "left" in h1.lower() and "right" in h2.lower():
        ...
        return (f"<strong>Destro vs Canhoto</strong> — Serviço de {l2} isola o backhand de {l1}. "
                +("Em Clay o ritmo lento permite reposicionamento." if surf=="Clay"
...
def matches_tbl(matches, surface):
    ...
    for m in matches[:10]:
        ...
        surf=str(m["surface"])
        sc=scls.get(surf.lower(),"bb")
        star="★ " if surf.lower()==surface.lower() else ""
        rows+=f"<tr><td style='color:#8b949e;white-space:nowrap'>{m['date']}</td><td>{m['opponent']}</td><td style='color:#8b949e'>{m['tourney'][:22]}</td><td><span class='b {sc}'>{star}{surf}</span></td><td style='color:#8b949e;font-size:11px'>{m['round'][:15]}</td><td>{rb}</td><td style='font-family:JetBrains Mono,monospace;font-size:11px'>{m['score']}</td></tr>"
...
def h2h_tbl(h2h_raw):
    events = h2h_raw if isinstance(h2h_raw, list) else (h2h_raw.get("H2H",[]) if isinstance(h2h_raw, dict) else [])
    if not events:
    ...
    for ev in events[:8]:
        ...
        surf=str(ev.get("surface","Hard")).capitalize()
        sc=scls.get(surf.lower(),"bb")
        scores=ev.get("scores",[])
        sc_str=" ".join([f"{s.get('score_first','?')}-{s.get('score_second','?')}" for s in scores if isinstance(s,dict)])
        date=str(ev.get("event_date",""))[:10]
        tourney=str(ev.get("tournament_name",ev.get("league_name","?")))[:24]
        rows+=f"<tr><td style='color:#8b949e'>{date}</td><td style='font-weight:600;color:#30d158'>{winner}</td><td style='color:#8b949e'>{tourney}</td><td><span class='b {sc}'>{surf}</span></td><td style='font-family:JetBrains Mono,monospace;font-size:11px'>{sc_str}</td></tr>"
    if not rows:
        st.markdown('<div style="color:#484f58;padding:8px">Sem confrontos directos encontrados.</div>', unsafe_allow_html=True); return
...
def main():
    ...
    with c1:
        ...
        p1i=st.text_input("p1","",placeholder="Ex: Sinner, Fucsovics...",label_visibility="collapsed")
    with c2:
        st.markdown('<div class="sl">Jogador 2</div>', unsafe_allow_html=True)
        p2i=st.text_input("p2","",placeholder="Ex: Tabilo, Alcaraz...",label_visibility="collapsed")
    with c3:
    ...
    st.markdown("""<div class="ib">💡 <strong>Dados reais 2025/2026</strong> via api-tennis.com —
      Escreve o apelido: <em>Sinner</em>, <em>Alcaraz</em>, <em>Fucsovics</em>, <em>Tabilo</em>, <em>Djokovic</em>...
    ...
    with st.spinner("A pesquisar jogadores..."):
        r1=search_player(p1i.strip()); r2=search_player(p2i.strip())

    if not r1: st.error(f"'{p1i}' não encontrado."); return
    if not r2: st.error(f"'{p2i}' não encontrado."); return

    p1=r1[0]; p2=r2[0]
    p1k=str(p1.get("player_key","")); p2k=str(p2.get("player_key",""))
    p1n=str(p1.get("player_name","?")); p2n=str(p2.get("player_name","?"))
    p1h=str(p1.get("player_hand","Right")); p2h=str(p2.get("player_hand","Right"))
    p1c=str(p1.get("player_country","")); p2c=str(p2.get("player_country",""))
    ...
    p1m=parse_matches(p1ev,p1n,surface); p2m=parse_matches(p2ev,p2n,surface)
    p1sf,p1sw,p1sc=surface_form(p1m,surface); p2sf,p2sw,p2sc=surface_form(p2m,surface)
    p1me=mental(p1m); p2me=mental(p2m)
    p1st=season_stats(p1det,surface); p2st=season_stats(p2det,surface)
    pct1,pct2,verdict,side=edge_score(p1sf,p2sf,p1me,p2me,p1st,p2st,p1n,p2n)
    htxt=hand_txt(p1h,p2h,p1n,p2n,surface)

    h2h_events=h2h_raw if isinstance(h2h_raw,list) else (h2h_raw.get("H2H",[]) if isinstance(h2h_raw,dict) else [])
    p1n_part=p1n.split(".")[-1].strip().lower()
    hw1=sum(1 for e in h2h_events if isinstance(e,dict) and e.get("event_winner")=="First Player" and p1n_part in str(e.get("event_first_player","")).lower())
    ...
    st.markdown(f"""<div class="card">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:14px;flex-wrap:wrap;gap:20px">
        <div style="text-align:center;flex:1;min-width:160px">
          ...
          <div style="font-size:11px;color:#8b949e;margin-bottom:10px">{p1st['surf_won']}V/{p1st['surf_lost']}D em {surface} · {p1st['titles']} títulos</div>
        ...
        <div style="text-align:center;flex:1;min-width:160px">
          ...
          <div style="font-size:11px;color:#8b949e;margin-bottom:10px">{p2st['surf_won']}V/{p2st['surf_lost']}D em {surface} · {p2st['titles']} títulos</div>
    ...
    st.markdown(f"""<div class="card">
      ...
      <div style="display:flex;justify-content:space-between;margin-top:5px">
        <div style="font-size:10px;color:#8b949e;font-family:'JetBrains Mono',monospace">{p1n.split(".")[-1].strip().upper()}</div>
        <div style="font-size:10px;color:#8b949e;font-family:'JetBrains Mono',monospace">{p2n.split(".")[-1].strip().upper()}</div>
      </div></div>""", unsafe_allow_html=True)
    ...
    with t1:
        ca,cb=st.columns(2)
        for col,pn,pm,psf,psw,psc,pst in [(ca,p1n,p1m,p1sf,p1sw,p1sc,p1st),(cb,p2n,p2m,p2sf,p2sw,p2sc,p2st)]:
            with col:
                l10=pm[:10]; wc=sum(1 for m in l10 if m["result"]=="W"); tot=len(l10)
                wrp=round(wc/tot*100) if tot>0 else 0
                rs=f"#{pst['rank']}" if pst['rank'] else "NR"
    ...
    with t2:
        ca,cb=st.columns(2)
        for col,pn,pst,psf in [(ca,p1n,p1st,p1sf),(cb,p2n,p2st,p2sf)]:
            with col:
                tw=pst["total_won"]; tl=pst["total_lost"]
                wr=round(tw/(tw+tl)*100,1) if (tw+tl)>0 else 0
                swr=round(pst["surf_won"]/(pst["surf_won"]+pst["surf_lost"])*100,1) if (pst["surf_won"]+pst["surf_lost"])>0 else 0
                st.markdown(f"""<div class="card"><div class="sl">{pn} — Época Actual</div>
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:12px">
                    ...
                    <div style="background:var(--el);border-radius:8px;padding:12px;text-align:center">
                      <div style="font-size:20px;font-weight:700;color:#30d158">{pst['titles']}</div>
                  ...
                  <div style="margin-top:10px;background:var(--el);border-radius:8px;padding:12px">
                    ...
                    <div style="font-size:18px;font-weight:700;color:{'#30d158' if swr>=50 else '#ff453a'}">{pst['surf_won']}V / {pst['surf_lost']}D &nbsp;·&nbsp; {swr}%</div>
                  </div></div>""", unsafe_allow_html=True)
        ...
        st.markdown(f"""<div class="card"><div class="sl">Comparação Directa</div>
          <div style="display:flex;justify-content:space-between;padding:10px 0 12px;border-bottom:1px solid var(--border);margin-bottom:4px">
            <span style="font-weight:600">{p1n.split(".")[-1].strip()}</span>
            <span style="font-size:10px;color:#484f58;letter-spacing:1px">MÉTRICA</span>
            <span style="font-weight:600">{p2n.split(".")[-1].strip()}</span></div>""", unsafe_allow_html=True)
    ...
    with t3:
        ...
        elif p1me["unstable"]: mv=f"🎯 {p2n.split('.')[-1].strip()} tem vantagem psicológica."; mc2=""
        elif p2me["unstable"]: mv=f"🎯 {p1n.split('.')[-1].strip()} tem vantagem psicológica."; mc2=""
    ...
    with t4:
        st.markdown(f"""<div class="card"><div class="sl">Head-to-Head Histórico</div>
          <div style="display:flex;align-items:center;justify-content:center;gap:40px;margin:14px 0;padding:14px;background:var(--el);border-radius:10px">
            <div style="text-align:center">
              ...
              <div style="font-size:12px;color:#8b949e">{p1n.split(".")[-1].strip()}</div></div>
            ...
            <div style="text-align:center">
              ...
              <div style="font-size:12px;color:#8b949e">{p2n.split(".")[-1].strip()}</div></div>
          </div>
        ...
        st.markdown(f"""<div class="card"><div class="sl">Análise de Lateralidade</div>
          <div style="display:flex;justify-content:center;gap:36px;padding:14px;background:var(--el);border-radius:10px;margin:12px 0">
            <div style="text-align:center">
              <div style="font-size:26px">{'🤚' if 'left' in p1h.lower() else '✋'}</div>
              <div style="font-weight:600;margin-top:4px">{p1n.split(".")[-1].strip()}</div>
              <div style="font-size:11px;color:#8b949e">{p1h}</div></div>
            <div style="display:flex;align-items:center;color:#484f58">⟷</div>
            <div style="text-align:center">
              <div style="font-size:26px">{'🤚' if 'left' in p2h.lower() else '✋'}</div>
              <div style="font-weight:600;margin-top:4px">{p2n.split(".")[-1].strip()}</div>
    ...
    with t5:
        ...
        mn=""
        if p1me["unstable"] and side!="p1": mn=f" ⚠ {p1n.split('.')[-1].strip()} perdeu os últimos 3 tie-breaks."
        elif p2me["unstable"] and side!="p2": mn=f" ⚠ {p2n.split('.')[-1].strip()} perdeu os últimos 3 tie-breaks."
        ...
        hn=""
        if hw1+hw2>0:
            if hw1>hw2: hn=f" H2H: {p1n.split('.')[-1].strip()} lidera {hw1}-{hw2}."
            elif hw2>hw1: hn=f" H2H: {p2n.split('.')[-1].strip()} lidera {hw2}-{hw1}."
          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:20px">
            <div style="flex:2;min-width:240px">
              ...
              <div style="font-size:14px;color:#c9d1d9;line-height:1.8">{sn}{mn}{hn}<br>
                Edge: <strong style="color:{'#30d158' if side=='p1' else '#f0f6fc'}">{p1n.split(".")[-1].strip()} {pct1}%</strong> vs
                <strong style="color:{'#30d158' if side=='p2' else '#f0f6fc'}">{p2n.split(".")[-1].strip()} {pct2}%</strong>. {mkt}</div></div>
            <div style="flex:1;min-width:130px;text-align:center;padding:22px;background:rgba(0,0,0,.2);border-radius:12px">
              ...
              <div style="font-size:12px;font-weight:600;color:#f0f6fc;margin-top:4px">{'⚡ '+favn.split(".")[-1].strip() if side!='neutral' else '🎲 COIN FLIP'}</div></div></div>
